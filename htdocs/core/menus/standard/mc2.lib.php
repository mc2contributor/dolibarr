...
/**
 *  \file		htdocs/core/menus/standard/mc2.lib.php
 *  \brief		Library for file mc2 menus
 */
require_once DOL_DOCUMENT_ROOT.'/core/class/menubase.class.php';


/**
 * Core function to output top menu mc2
 *
 * @param 	DoliDB	$db				Database handler
 * @param 	string	$atarget		Target (Example: '' or '_top')
 * @param 	int		$type_user     	0=Menu for backoffice, 1=Menu for front office
 * @param  	array	$tabMenu        If array with menu entries already loaded, we put this array here (in most cases, it's empty). For mc2 menu, it contains menu entries loaded from database.
 * @param	Menu	$menu			Object Menu to return back list of menu entries
 * @param	int		$noout			1=Disable output (Initialise &$menu only).
 * @param	string	$mode			'top', 'topnb', 'left', 'jmobile'
 * @return	int						0
 */
function print_mc2_menu($db, $atarget, $type_user, &$tabMenu, &$menu, $noout = 0, $mode = '')
{
	...
	$menu_arr[] = array(
		...
		//OR: 'prefix' => img_picto('', 'mrp', 'class="fa-fw paddingright pictofixedwidth"'),
		'session' => ((!empty($_SESSION["mainmenu"]) && $_SESSION["mainmenu"] == "products") ? 0 : 1),
		'loadLangs' => array("products", "stocks", "mrp"),
		'submenus' => array(),
	);
	...
	// Billing - Financial, Accounting, Bank
	$tmpentry = array(
		'enabled'=>(isModEnabled('facture') ||
			...
			isModEnabled('comptabilite') || isModEnabled('accounting') ||
			isModEnabled('asset') || isModEnabled('intracommreport') ||
			isModEnabled('banque') || isModEnabled('prelevement')
			) ? 1 : 0,
		'perms'=>($user->hasRight('facture',  'lire') || $user->hasRight('don',  'contact', 'lire')
			...
			|| $user->hasRight('compta',  'resultat', 'lire') || $user->hasRight('accounting',  'comptarapport', 'lire') || $user->hasRight('accounting',  'mouvements', 'lire') || $user->hasRight('asset',  'read') || $user->hasRight('intracommreport',  'read')
			|| $user->hasRight('banque',  'lire') || $user->hasRight('prelevement',  'lire') || $user->hasRight('paymentbybanktransfer',  'read')
		),
		'module'=>'facture|supplier_invoice|don|tax|salaries|loan|comptabilite|accounting|asset|intracommreport|banque|prelevement|paymentbybanktransfer'
	);
	$menu_arr[] = array(
		'name' => 'Compta',
		'link' => '/compta/index.php?mainmenu=accountancy&amp;leftmenu=',
		...
		//OR: 'prefix' => img_picto('', 'accountancy', 'class="fa-fw paddingright pictofixedwidth"'),
		'session' => ((!empty($_SESSION["mainmenu"]) && $_SESSION["mainmenu"] == "accountancy") ? 0 : 1),
		'loadLangs' => array("compta", "accountancy", "assets", "intracommreport", "banks"),
		'submenus' => array(),
	);
	...
	$menuArbo = new Menubase($db, 'mc2');

	$newTabMenu = $menuArbo->menuTopCharger('', '', $type_user, 'mc2', $tabMenu); // Return tabMenu with only top entries
...
/**
 * Core function to output left menu mc2
 ...
 */
function print_left_mc2_menu($db, $menu_array_before, $menu_array_after, &$tabMenu, &$menu, $noout = 0, $forcemainmenu = '', $forceleftmenu = '', $moredata = null, $type_user = 0)
{
	...
	if ($mainmenu) {	// If this is empty, loading hard coded menu and loading personalised menu will fail
		...
		$menuArbo = new Menubase($db, 'mc2');
		$newmenu = $menuArbo->menuLeftCharger($newmenu, $mainmenu, $leftmenu, (empty($user->socid) ? 0 : 1), 'mc2', $tabMenu);
		//var_dump($newmenu->liste);    //
...
function get_left_menu_accountancy($mainmenu, &$newmenu, $usemenuhider = 1, $leftmenu = 'none', $type_user = 0)
{
	...
	if ($mainmenu == 'accountancy') {
		...
		if (isModEnabled('accounting')) {
			...
			// Configuration
			$newmenu->add("/accountancy/index.php?leftmenu=accountancy_admin", $langs->trans("Accounting Setup"), 0, $user->hasRight('accounting',  'chartofaccount'), '', $mainmenu, 'accountancy_admin', 1, '', '', '', img_picto('', 'technic', 'class="paddingright pictofixedwidth"'));
