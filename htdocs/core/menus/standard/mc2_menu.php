...
/**
 *	\file       htdocs/core/menus/standard/mc2_menu.php
 *	\brief      Menu mc2 manager
 */
...
class MenuManager
{
	...
	public function loadMenu($forcemainmenu = '', $forceleftmenu = '')
	{
		...
		$tabMenu = array();
		$menuArbo = new Menubase($this->db, 'mc2');
		$menuArbo->menuLoad($mainmenu, $leftmenu, $this->type_user, 'mc2', $tabMenu);
	...
	public function showmenu($mode, $moredata = null)
	{
		...
		require_once DOL_DOCUMENT_ROOT.'/core/menus/standard/mc2.lib.php';
		...
		if (empty($conf->global->MAIN_MENU_INVERT)) {
			if ($mode == 'top') {
				print_mc2_menu($this->db, $this->atarget, $this->type_user, $this->tabMenu, $this->menu, 0, $mode);
			}
			if ($mode == 'left') {
				print_left_mc2_menu($this->db, $this->menu_array, $this->menu_array_after, $this->tabMenu, $this->menu, 0, '', '', $moredata, $this->type_user);
			}
		} else {
		...
		if ($mode == 'topnb') {
			print_mc2_menu($this->db, $this->atarget, $this->type_user, $this->tabMenu, $this->menu, 1, $mode); // no output
			return $this->menu->getNbOfVisibleMenuEntries();
		}

		if ($mode == 'jmobile') {     // Used to get menu in xml ul/li
			print_mc2_menu($this->db, $this->atarget, $this->type_user, $this->tabMenu, $this->menu, 1, $mode); // Fill this->menu that is empty with top menu
			...
			foreach ($this->menu->liste as $key => $val) {		// $val['url','titre','level','enabled'=0|1|2,'target','mainmenu','leftmenu'
				...
				if ($val['enabled'] == 1) {
					...
					$tmpleftmenu = 'all';
					$submenu = new Menu();
					print_left_mc2_menu($this->db, $this->menu_array, $this->menu_array_after, $this->tabMenu, $submenu, 1, $tmpmainmenu, $tmpleftmenu, null, $this->type_user); // Fill $submenu (example with tmpmainmenu='home' tmpleftmenu='all', return left menu tree of Home)
